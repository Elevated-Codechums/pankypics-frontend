...
export default function Photos() {
  ...
  const mutation = useMutation({
    mutationFn: async (formData: FormValues) => {
      ...
      const response = await axiosClient.post("album/upload/photos", data, {
        headers: {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    ...
    setValue("photoCaptions", [...getValues("photoCaptions"), ...newCaptions]);
  };
  ...
  return (
    <div className={cn("flex items-center min-h-screen gap-4 py-20 ")}>
      ...
      >
        ...
        <Input type="text" {...register("albumName", { required: true })} />
        ...
        <div
          ...
          className={cn(
            ...
            isDragActive ? "border-dashed bg-gray-700" : "bg-gray-800"
          )}
        >
          <input {...getInputProps()} />
          {isDragActive ? (
          ...
          <div>
            {mutation.isError && (
              <p className="text-red-500">
                Error uploading: {(mutation.error as Error).message}
              </p>
            )}
            {mutation.isSuccess && (
