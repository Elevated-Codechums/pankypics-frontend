...
const EditPage = () => {
  ...
  const { isLoading, error } = useQuery<FormData, Error>({
    queryKey: ["pages", slug],
    queryFn: async () => {
      const response = await axiosClient.get(`/pages/${slug}`);
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      await axiosClient.put(`/pages/${data.id}`, data);
    },
    onSuccess: () => {
  ...
  if (error) return <div>Error fetching page: {(error as Error).message}</div>;
